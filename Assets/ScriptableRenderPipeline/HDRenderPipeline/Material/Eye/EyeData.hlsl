...
//UV Refraction
//----------------------------
float2 EyeUvs(float2 uv, float irisDepth, float irisRadius, float3 viewTS)
{
    float irisDist = 0.25+0.03*irisDepth- dot(uv - 0.5, uv - 0.5);
    irisDist = saturate(irisDist - .22) * (1.0 / .22);

    float2 uvOrig = uv;
    uv -= viewTS.xy * irisDist;
    uv = ((uv - 0.5) * (1 - irisDist * irisRadius * 10)) + 0.5;

    return uv;
}
//----------------------------
/*
float2 EyeUvs(float2 uv, float irisDepth, float irisRadius, float IOR, float3 normalWS, float3 viewWS, inout float4 color)
{

    // sqrt(0.25 - x^2) -3/4*0.5
    float2 centered_uv = uv - float2(0.5, 0.5);
    float r_center_sq = dot(centered_uv, centered_uv);
    float eye_radius = 0.9;
    float height = saturate(sqrt(eye_radius * eye_radius - r_center_sq) - 0.97 * eye_radius) * irisDepth * 20;
    //float height = irisDepth * saturate( 1.0 - 18.4 * irisRadius * irisRadius ); 
    ...
    float mask = saturate(SAMPLE_TEXTURE2D(_MaskMap, sampler_MaskMap, uv).r - 0.1);
    float2 eyeUV = uv;

    float cosAlpha = dot(float3(0,0,1), -refractedW);
    color = float4(cosAlpha, cosAlpha, cosAlpha, 1);

	float dist = height / cosAlpha;
	float3 offsetW = dist * refractedW;
	float2 offsetL = mul((float3x2)unity_WorldToObject, offsetW);
...
void GetSurfaceAndBuiltinData(FragInputs input, float3 V, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData)
{
	...
	input.texCoord0 = EyeUvs(input.texCoord0, _IrisDepth, _IrisRadius, /*TODO: TS View */ mul(input.worldToTangent, V));
	//---------------------
